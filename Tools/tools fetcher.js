...
var docbodye = document.getElementsByTagName("BODY")[0];
if (window.location.href.match('quirytool=ist') != null) {
    if (id('webtool-place') != null) {
        ...
        document.querySelector('[type="image/x-icon"]').href = 'https://mnzrch.github.io/SkytoSeeTools/Tools/assests/fav.ico';
...
if (window.location.href.match('quirytool=ehsaas') != null) {
    if (id('webtool-place') != null) {
        let webtoolcon = '<span id="fix-overlay-ctrl" class="margin-5 btnv1" style="cursor: pointer;">⨯</span><iframe src="https://8171.pass.gov.pk" frameborder="0" class="ehsaas-frame margin-0"></iframe>';
        id('webtool-place').innerHTML = webtoolcon;
        docbodye.style.overflow = 'hidden';
        if (window.location.href.match('mywebapp') != null) {
            ...
            id('fix-overlay-ctrl').addEventListener('click', () => {
                ...
                docbodye.style.overflowX = 'hidden';
                docbodye.style.overflowY = 'auto';
            })
        }
        setTimeout(() => {
